...
// Returns whether the body is a custom inline type, or pre-defined
	...
	for _, requestPath := range SortedPathsKeys(swagger.Paths) {
		...
		for _, opName := range SortedOperationsKeys(pathOps) {
			...
			if op.RequestBody != nil {
				...
				if found {
					bodySchema, err := GenerateGoSchema(content.Schema, []string{op.OperationID})
					if err != nil {
					...
					if bodyOrRef.Ref != "" {
						opDef.Body.Schema.RefType = bodyOrRef.Ref
					}
				}
			}
